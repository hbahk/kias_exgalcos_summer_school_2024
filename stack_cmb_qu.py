...
def process_chunk(
    qsm, usm, lons, lats, xsize, shared_qrms, shared_urms, start_index, end_index, nside
):
    local_qrms, local_urms = [], []
    ipix = np.arange(hp.nside2npix(nside))
    for i in trange(start_index, end_index):
        local_qsm = hp.visufunc.gnomview(
            qsm,
            rot=(lons[i], lats[i], 0),
            xsize=xsize,
        ...
        local_usm = hp.visufunc.gnomview(
            usm,
            rot=(lons[i], lats[i], 0),
            xsize=xsize,
            return_projected_map=True,
        ...
        local_imp = hp.visufunc.gnomview(
            ipix,
            rot=(lons[i], lats[i], 0),
            xsize=xsize,
            return_projected_map=True,
        ...
        local_qrm = -local_qsm * np.cos(2 * local_phi) - local_usm * np.sin(
            2 * local_phi
        )
        local_urm = local_qsm * np.sin(2 * local_phi) - local_usm * np.cos(
            2 * local_phi
        )
        ...
        local_qrms.append(local_qrm)
        local_urms.append(local_urm)
    # Append results to the shared list
    shared_qrms.extend(local_qrms)
    shared_urms.extend(local_urms)
...
def main():
    ...
    for i in range(num_processes):
        ...
        end_index = (
            start_index + chunk_size if i < num_processes - 1 else len(hlons1) - 1
        )
        p = Process(
    ...
    hqmaps = np.array(hqmaps)
    humaps = np.array(humaps)
    cqmaps = np.array(cqmaps)
    cumaps = np.array(cumaps)
    np.save("hqmaps.npy", hqmaps)
    np.save("humaps.npy", humaps)
    np.save("cqmaps.npy", cqmaps)
    np.save("cumaps.npy", cumaps)
